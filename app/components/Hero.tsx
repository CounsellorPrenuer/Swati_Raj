...
export default function Hero({ siteSettings, founder }: { siteSettings?: any; founder?: any }) {
  const title = siteSettings?.title || 'SWAPATH - FIND YOUR WAY'
  const description = siteSettings?.description || 'Expert career guidance and admission counselling for students, parents, schools, and colleges.'
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6">
              {title.replace('SWAPATH - ', '').replace('FIND YOUR WAY', 'Find Your Way to')} <span className="text-gradient">Success</span>
            </h1>
